      ...
      <Card className="from-primary/5 via-card to-card border-primary/20 bg-gradient-to-br">
        <CardContent className="pt-5 pb-5">
          <form onSubmit={handleAdd} className="flex flex-col gap-2 sm:flex-row">
            <Input
              type="text"
              aria-label="Ticker a agregar al watchlist"
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="space-y-3 py-8 text-center">
            <p className="text-destructive text-sm">No se pudo cargar tu watchlist.</p>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {entries.map(({ item, quote, recentCloses }) => {
            ...
            return (
              <div
                key={item.id}
                className="group bg-card hover:border-primary/40 border-border relative overflow-hidden rounded-lg border transition-all hover:shadow-sm"
              >
                ...
                >
                  ...
                  {recentCloses && recentCloses.length > 1 ? (
                    <div className="border-border/60 -mx-4 -mb-4 border-t px-4 py-3">
                      <div className="text-muted-foreground mb-1 flex items-baseline justify-between text-[10px] tracking-wider uppercase">
                        <span>Últimos 30 días</span>
                        <span className={isPositive ? "text-success" : "text-destructive"}>
                          {isPositive ? "▲" : "▼"}
                        </span>
                      </div>
                      <SparkLine data={recentCloses} width={280} height={40} className="w-full" />
                    </div>
                  ) : (
                    <div className="text-muted-foreground text-xs">Sin histórico disponible</div>
                  )}
